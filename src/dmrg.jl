...
function sweep(mps, mpo, HL, HR, CL, CR, prec,canonicity, orth=[])
    ...
    var = H2 - E^2
    return mps, E, var, switch_canonicity(canonicity)
end
"""
    eigenstates(mps, hamiltonian, prec, n)

Return the `n` eigenstates with the lowest energy
"""
...
function initializeHLR(mps,mpo,HL,HR)
    ...
    for j=L-1:-1:1
        @tensoropt (-1,1,-3,3) HR[j][-1,-2,-3] := conj(mps[j+1][-1,4,1])*mpo[j+1][-2,4,5,2]*mps[j+1][-3,5,3]*HR[j+1][1,2,3]
    end
    for j=2:L
        @tensoropt (1,3,-1,-3) HL[j][-1,-2,-3] := HL[j-1][1,2,3]*conj(mps[j-1][1,4,-1])*mpo[j-1][2,4,5,-2]*mps[j-1][3,5,-3]
    end
end
...
function updateHLR(mps,mpo,HL,HR,i,dir)
    L = length(mps)
    if dir==:right
        @tensoropt (1,3,-1,-3) HL[i+1][-1,-2,-3] := HL[i][1,2,3]*conj(mps[i][1,4,-1])*mpo[i][2,4,5,-2]*mps[i][3,5,-3]
    end
    if dir==:left
        @tensoropt (-1,1,-3,3) HR[i-1][-1,-2,-3] := conj(mps[i][-1,4,1])*mpo[i][-2,4,5,2]*mps[i][-3,5,3]*HR[i][1,2,3]
    end
end
...
function getHeff(mps,mpo,HL,HR,i)
    L=length(mps)
    @tensoropt (-1,-4,-3,-6) Heff[:] := HL[i][-1,1,-4]*mpo[i][1,-2,-5,2]*HR[i][-3,2,-6]
    return Heff
end

function HeffMult(tensor,mpo,HL,HR)
    @tensoropt (-1,4,6,-3) temp[:] := HL[-1,1,4]* mpo[1,-2,5,2] *tensor[4,5,6] *HR[-3,2,6]
    return temp
end
